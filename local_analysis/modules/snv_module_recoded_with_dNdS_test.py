...
def generate_cnn_filter_table(all_p, filt_res, dpt, dlab, dprob, dir_output, cmt_p, dgap, my_cmt, filter_labels, min_cov_filt):
    o=open(dir_output+'/snv_table_cnn_plus_filter.txt','w+')
    header = (
        'genome_pos\tPred_label\tCNN_pred\tWideVariant_pred\tCNN_prob\t'
        f"{filter_labels['qual']}\t{filter_labels['cov']}\t{filter_labels['maf']}\t"
        f"{filter_labels['indel']}\t{filter_labels['mfas']}\t{filter_labels['mmcp']}\t{filter_labels['cpn']}\t"
        'Fix_filter\tWhether_recomb\tFraction_ambigious_samples\tGap_filter\n'
    )
    o.write(header)
...
def generate_html_with_thumbnails(input_file, output_file, chart_dir,contig_names=None, filter_labels=None):
    df = pd.read_csv(input_file, sep='\t')
    if contig_names is not None and 'contig_idx' in df.columns:
        def idx_to_name(x):
            try:
                ix = int(x)
                if 0 < ix <= len(contig_names):
                    return contig_names[ix - 1]
            except Exception:
                pass
            return x
        df['contig_idx'] = df['contig_idx'].apply(idx_to_name)
    d = {}
    ...
    if filter_labels is None:
        filter_labels = {
            ...
            'cov': 'Cov_filter (<5)',
            'maf': 'MAF_filter (>0.85)',
            'indel': 'Indel_filter (<0.33)',
            'mfas': 'MFAS_filter (1)',
            'mmcp': 'MMCP_filter (5)',
            'cpn': 'CPN_filter (4,7)'
        }
    qual_col = filter_labels['qual']
    cov_col = filter_labels['cov']
    maf_col = filter_labels['maf']
    indel_col = filter_labels['indel']
    mfas_col = filter_labels['mfas']
    mmcp_col = filter_labels['mmcp']
    cpn_col = filter_labels['cpn']
    ...
    with open(output_file, 'w') as f:
        ...
        for idx, row in df.iterrows():
            ...
            <th class="pred">{qual_col}</th>
            <th class="pred">{cov_col}</th>
            '''
            f.write(html_p1+'\n')
            ...
            <th class="pred">{indel_col}</th>
            <th class="pred">{mfas_col}</th>
            <th class="pred">{mmcp_col}</th>
            <th class="pred">{cpn_col}</th>
        </tr>
        <tr>
