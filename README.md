...
⚠️⚠️⚠️: Job interruptions may be caused by issues with specific compute nodes (e.g. Timelimit or QOSMaxSubmitJobPerUserLimit). The profile now submits through `snake_pipeline/scripts/safe_sbatch.py`, which retries when the cluster reports `QOSMaxSubmitJobPerUserLimit` and optionally waits until your queue drops below `ACCUSNV_SUBMIT_LIMIT`. You can still explicitly limit parallel submissions by exporting `ACCUSNV_SUBMIT_LIMIT=<maxSubmitJob>` before calling `sbatch scripts/run_snakemake.slurm`, or by editing `<output_dir>/conf/config.yaml` to set `jobs: <maxSubmitJob>`.

  • **Why you may see `QOSMaxSubmitJobPerUserLimit` even with a low `jobs` setting:** Slurm enforces the submit quota against the total number of *running + pending* jobs in the QoS/account, not just the jobs that are allowed to start concurrently. If you already have many pending jobs from other workflows—or if the pipeline rapidly generates hundreds of future tasks—Snakemake can momentarily queue enough jobs to exceed the QoS ceiling, triggering the error despite `jobs` being set to 8. Use `squeue -u $USER` to inspect how many jobs the scheduler currently tracks for you. If that count is close to the site limit (e.g. ~500) reduce the outstanding queue by cancelling old jobs or export `ACCUSNV_SUBMIT_LIMIT=<safe_threshold>` so `safe_sbatch.py` waits before submitting more work.

  • **When `candidate_mutation_table` gets OOM-killed:** Building the candidate matrix scales with `(num_samples × num_candidate_sites × num_stats)`. Large cohorts (e.g. 1,200 isolates with ~800k candidate sites) require hundreds of gigabytes if everything is kept in RAM. If you see `Killed` followed by `Detected 1 oom_kill event` in Slurm logs, rerun on a node with more memory, or trim the workload: edit the `candidate_mutation_table` rule in `snake_pipeline/Snakefile` to drop the `-n` and `-c` options (skip coverage matrices) and/or lower the `-t` argument in `build_candidate_mutation_table.py` (e.g. `-t 8`) to keep the intermediate arrays smaller. As of this update the script streams large arrays to temporary on-disk memmaps, so the peak RAM pressure drops dramatically, but you will still need fast scratch storage for the temporary `.memmap` files and several hundred gigabytes of quota for the final `.npz` outputs when processing very large cohorts.
