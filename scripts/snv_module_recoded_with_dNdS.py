...
NTs_to_int_dict = {'N': 0, 'n': 0, 'A': 1, 'a': 1, 'T': 2, 't': 2, 'C': 3, 'c': 3, 'G': 4,
                   'g': 4}  # must convert lowercase NTs to ints
...
def nts2ints(np_array_of_NTs):
    ...
    return np.vectorize(NTs_to_int_dict.get)(np_array_of_NTs,
                                             0)  # vectorize much faster than looping; requires numpy array input
...
def ints2nts(np_array_of_ints):
    ...
    return np.vectorize(int_to_NTs_dict.__getitem__)(
        np_array_of_ints)  # vectorize much faster than looping; require numpy array input
...
def nts_rev_comp(np_array_of_NTs):
    ...
    NTs_complement = np.vectorize(NTs_complement_dict.get)(np_array_of_NTs,
                                                           'N')  # vectorize much faster than looping; require numpy array input
...
def read_old_candidate_mutation_table_pickle_gzip(file_cmt_pickle_gz):
    ...
    # Read file
    [quals, p, counts, in_outgroup, sampleNames, indel_counter] = read_candidate_mutation_table_pickle_gzip(
        file_cmt_pickle_gz)
...
class cmt_data_object:
    ...
    def __init__(self, sample_names_list, in_outgroup_bool, positions_list, counts_array, quals_array,
                 indel_stats_array, my_dataset_name='My candidate mutation table'):
        '''
        ...
        * Dimensions of arrays must confrom to: num_samples x num_pos x (third dimension where applicable).
        ...
        try:
            if in_outgroup_bool.dtype.type == np.bool_:
                if in_outgroup_bool.shape == (self.num_samples,):
                    ...
                    raise Exception("Outgroup boolean array dimensions are " + str(
                        in_outgroup_bool.shape) + ", but should be (" + str(self.num_samples) + ",).")
            else:
        ...
        try:
            if np.issubdtype(counts_array.dtype, np.integer):
                if counts_array.shape == (self.num_samples, self.num_pos, 8):
                    ...
                    raise Exception(
                        "Counts array dimensions are " + str(counts_array.shape) + ", but should be (" + str(
                            self.num_samples) + ", " + str(self.num_pos) + ", 8).")
            else:
        ...
        try:
            if np.issubdtype(quals_array.dtype, np.integer):
                if quals_array.shape == (self.num_samples, self.num_pos):
                    ...
                    raise Exception("Quals array dimensions are " + str(quals_array.shape) + ", but should be (" + str(
                        self.num_samples) + ", " + str(self.num_pos) + ").")
            else:
        ...
        try:
            if np.issubdtype(indel_stats_array.dtype, np.integer):
                if indel_stats_array.shape == (self.num_samples, self.num_pos, 2):
                    ...
                    raise Exception(
                        "Indel stats array dimensions are " + str(counts_array.shape) + ", but should be (" + str(
                            self.num_samples) + ", " + str(self.num_pos) + ", 2).")
            else:
        ...
        self.major_nt_freq[np.isnan(
            self.major_nt_freq)] = 0  # set major allele frequency to zero to indicate there is no data; leave minor allele frequency as nan
        ...
        self.major_nt = 1 + np.squeeze(counts_argsort[:, :, 3:4],
                                       axis=2)  # add major alelle attribute # 3:4 necessary to maintain 3d structure # +1 necessary because 0=N and 1-4=ATCG
        # self.major_nt_fwd = 1 + np.squeeze( counts_argsort_fwd[:,:,3:4],axis=2 )
        # self.major_nt_rev = 1 + np.squeeze( counts_argsort_rev[:,:,3:4],axis=2 )
        self.minor_nt = 1 + np.squeeze(counts_argsort[:, :, 2:3],
                                       axis=2)  # add minor allele attribute # 2:3 necessary to maintain 3d structure # +1 necessary because 0=N and 1-4=ATCG
        ...
        counts_minor_fwd = np.squeeze(counts_sort_fwd[:, :, 2:3],
                                      axis=2)  # number of reads for next most common nucleotide
        counts_minor_rev = np.squeeze(counts_sort_rev[:, :, 2:3],
                                      axis=2)  # number of reads for next most common nucleotide
    ...
    def filter_samples(self, samples_to_keep_bool):
        ...
        try:
            if (samples_to_keep_bool.dtype.type == np.bool_) and (samples_to_keep_bool.size == self.num_samples):
                ...
                # print results
                print(
                    "Number of samples in candidate mutation table reduced from " + str(num_samples_old) + " to " + str(
                        self.num_samples) + ".")
            else:
                raise Exception(
                    "Argument samples_to_keep_bool must be a numpy array of booleans with size num_samples.")
        except AttributeError:
    ...
    def filter_positions(self, positions_to_keep_bool):
        ...
        try:
            if (positions_to_keep_bool.dtype.type == np.bool_) & (positions_to_keep_bool.size == self.num_pos):
                ...
                # print results
                print("Number of positions in candidate mutation table reduced from " + str(num_pos_old) + " to " + str(
                    self.num_pos) + ".")
            else:
    ...
    def copy(self):
        ...
        return cmt_data_object(self.sample_names, self.in_outgroup, self.p, self.counts, self.quals, self.indel_stats,
                               self.dataset_name)
...
class cov_data_object_simple:
    ...
    def __init__(self, raw_cov_mat, sample_names, genome_length, contig_starts, contig_names):
        ...
        if raw_cov_mat.shape != (self.num_samples, self.genome_length):
            raise Exception("Raw coverage array dimensions are " + str(raw_cov_mat.shape) + ", but should be (" + str(
                self.num_samples) + ", " + str(self.genome_length) + ").")
        ...
        np.divide(self.median_coverage_by_contig,
                  np.expand_dims(self.median_coverage_by_contig[:, idx_chromosomal_contig], 1), \
    ...
    def filter_samples(self, samples_to_keep_bool):
        ...
        try:
            if (samples_to_keep_bool.dtype.type == np.bool_) and (samples_to_keep_bool.size == self.num_samples):
                ...
                # print results
                print("Number of samples in coverage object reduced from " + str(num_samples_old) + " to " + str(
                    self.num_samples) + ".")
            else:
                raise Exception(
                    "Argument samples_to_keep_bool must be a numpy array of booleans with size num_samples.")
    ...
    def plot_heatmap_contig_copy_num(self, dir_save_fig, show, copynum_too_low_cutoff=0.5):
        ...
        masked_array = np.ma.masked_where(self.copy_number_by_contig < copynum_too_low_cutoff,
                                          self.copy_number_by_contig)
        ...
        tick_labels_for_colorbar = ['$\leq$' + str(copynum_too_low_cutoff), '1', '3', '6', '9',
                                    '$\geq$' + str(copynum_high_cutoff)]
...
class cov_data_object(cov_data_object_simple):
    ...
    def make_coverage_trace(self, contig_number, num_cov_bins, dir_save_fig, show):
        ...
        for idx in np.arange(num_cov_bins):
            ...
            cov_trace_data[:, idx] = np.mean(self.raw_cov_mat[:, bin_start_idx:bin_end_idx],
                                             axis=1) / self.get_median_cov_of_chromosome()
        ...
        # exit()
        plt.ylim(ymin=0,
                 ymax=np.maximum(1., 1.1 * np.nanmax(np.where(np.isinf(cov_trace_data), np.nan, cov_trace_data))))
        # exit()
...
class calls_object:
    ...
    def filter_samples(self, samples_to_keep_bool):
        ...
        try:
            if (samples_to_keep_bool.dtype.type == np.bool_) and (samples_to_keep_bool.size == self.num_samples):
                ...
                # print results
                print("Number of samples in calls object reduced from " + str(num_samples_old) + " to " + str(
                    self.num_samples) + ".")
            else:
                raise Exception(
                    "Argument samples_to_keep_bool must be a numpy array of booleans with size num_samples.")
    ...
    def filter_positions(self, positions_to_keep_bool):
        ...
        try:
            if (positions_to_keep_bool.dtype.type == np.bool_) & (positions_to_keep_bool.size == self.num_pos):
                ...
                # print results
                print("Number of positions in calls object reduced from " + str(num_pos_old) + " to " + str(
                    self.num_pos) + ".")
            else:
                raise Exception("Argument positions_to_keep_bool must be a numpy array of booleans with size num_pos.")
        except AttributeError:
    ...
    def filter_calls_by_sample(self, samples_to_filter):
        ...
        self.calls[samples_to_filter, :] = NTs_to_int_dict[
            'N']  # set all calls in bad samples to N by broadcasting boolean argument

    def filter_calls_by_position(self, positions_to_filter):
        ''' Filter calls in bad positions. '''
        self.calls[:, positions_to_filter] = NTs_to_int_dict[
            'N']  # set all calls in bad positions to N by broadcasting boolean argument
...
class reference_genome_object:
    ...
    def __init__(self, dir_ref_genome):
        ...
        # Read in FASTA file
        [self.contig_starts, self.contig_names, self.genome_length, self.genome_seq] = get_genome_stats_from_fasta(
            dir_ref_genome)
        ...
        if len(glob.glob(dir_ref_genome + '/*.gff*')) != 1:
            ...
            print(
                "WARNING! Reference genome object initialized with empty annotations because either no gff file was found or multple gff files were found!")
        else:
        ...
        [self.locus_tagnumbers, self.cds_indices] = tag_all_genomic_positions(self.annotations, self.genome_length,
                                                                              self.contig_starts)
    ...
    def p2contigpos(self, p):
        ...
        if len(self.contig_starts) > 1:  # if multiple contigs exist
            for next_contig_start in self.contig_starts[1:]:
                idx_of_contig = idx_of_contig + (
                            p >= next_contig_start)  # note: (p > i) adds 1 if true and adds 0 if false
    ...
    def get_ref_NTs(self, p):
        ...
        if p.ndim == 2:
            raise Exception(
                "Error! Argument p should not be two-dimensional. Convert contigpos to p using method contigpos2p.")
...
def parse_gff(dir_ref_genome, contig_names, ortholog_info_series=pd.Series(dtype='float64')):
    ...
    with open_gff(gff_file[0]) as gff_handle:
        possible_limits = examiner.available_limits(
            gff_handle)  # available_limits function gives a summary of feature attributes along with counts for the number of times they appear in the file
    ...
    for contig in contig_names:  # loop over contig annotations according to order in contig_names
        with open(gff_file[0]) as gff_handle:
            for rec in GFF.parse(gff_handle,
                                 limit_info=limits):  # loop over every contig, but only grab attributes specified by [limits] to save memory
                    ...
                    if len(rec.features) > 0:
                        ...
                        for gene_feature in rec.features:
                            ...
                            if gene_feature.id != "" and gene_feature.type == 'CDS' and not ortholog_info_series.empty:
                                gene_dict['orthologtag'] = ortholog_info_series[
                                    ortholog_info_series.str.findall(gene_feature.id).str.len() == 1].index[0]
                            ...
                            gene_dict['indices'] = [get_position(gene_feature.location.start) + 1,
                                                    get_position(gene_feature.location.end)]
                            ...
                            if gene_dict['type'] == "CDS":
                                ...
                                gene_dict[
                                    'translation'] = "."  # all non-CDS (RNA's or repeat regions) not translated (as those are sometimes also off-frame)
                        ...
                        df_sort = df_sort.sort_values(
                            by=['loc1'])  # sort pandas dataframe (annotation not necessarily sorted)
...
def tag_all_genomic_positions(anno_genes_ls, genome_length, contig_starts):
    ...
    # Initialize
    locus_tagnumbers = np.ones(genome_length,
                               dtype=float) * 0.5  # CDS tag ('tagnumber' from annotations dataframe) that is unique across all contigs; intergenic = 0.5; tRNA = 0 (since gff parser function sets tagnumber to zero for non-CDS annotations)
    cds_indices = np.ones(genome_length,
                          dtype=float) * 0.5  # CDS tag that is unique on a given contig only; intragenic = previous_idx+0.5
    ...
    for i, this_contig_df in enumerate(anno_genes_ls):
        ...
        gene_starts = this_contig_df[['loc1']].values.flatten() + contig_starts[
            i] - 1  # genome position indexing starts at 1
        gene_ends = this_contig_df[['loc2']].values.flatten() + contig_starts[
            i] - 1  # genome position indexing starts at 1

        # Mark positions across all genes except for the last one on the contig
        for j in range(len(gene_starts) - 1):
            locus_tagnumbers[(gene_starts[j] - 1):gene_ends[j]] = gene_tagnumbers[
                j]  # populate locus_tagnumbers across this gene
            cds_indices[(gene_starts[j] - 1):gene_ends[j]] = j + 1;  # populate cds_indices across this gene
            cds_indices[(gene_ends[j]):(gene_starts[
                                            j + 1] - 1)] = j + 1 + 0.5  # populate cds_indices between this gene and the next gene

        # Mark positions for last gene on the contig
        locus_tagnumbers[(gene_starts[-1] - 1):gene_ends[-1]] = gene_tagnumbers[
            -1]  # populate locus_tagnumbers across the last gene
        cds_indices[(gene_starts[-1] - 1):gene_ends[-1]] = len(
            gene_tagnumbers)  # populate cds_indices across the last gene

        # Mark remaining positions on contig
        if ((i + 1) < len(contig_starts)):
            cds_indices[gene_ends[-1]:contig_starts[i + 1] - 1] = len(
                gene_tagnumbers) + 0.5  # populate cds_indices after the last gene until the end of the contig
        else:  # last contig
            cds_indices[gene_ends[-1]:genome_length] = len(
                gene_tagnumbers) + 0.5  # populate cds_indices after the last gene until the end of the contig (same as the end of the genome)
...
def compute_expected_dnds(dir_ref_genome, mut_spec_prob, gene_nums_of_interest=None):
    ...
    def codons_in_genome(genome_directory, all_codons, subset_gene_nums=None):
        ...
        if subset_gene_nums is not None:  # if genes of interest are provided
            my_genes = [all_genes[i] for i in
                        subset_gene_nums]  # Take only specified gene numbers (NOTE: GENE NUMBERS DO NOT RESTART BETWEEN CONTIGS)
        ...
        for _, gene in my_genes.iterrows():
            ...
            next_codon_counts = Counter(
                ["".join(next_sequence[i:i + 3]) for i in range(0, len(next_sequence), 3)])  # tally codons in sequence
...
def filter_histogram(filter_value, filter_cutoff, filter_name, save_bool=False, dir_save_fig=os.getcwd(),
                     fig_file_name='snv_filter_histogram.png'):
    '''
...
def filter_samples_by_coverage(median_cov_by_sample, min_average_coverage_to_include_sample, sample_names,
                               plot_bool=False, dir_save_fig=os.getcwd()):
    '''
...
def filter_samples_by_ambiguous_basecalls(frac_ambig_basecalls_by_sample, max_frac_Ns_to_include_sample, sample_names,
                                          in_outgroup_bool, plot_bool=False, dir_save_fig=os.getcwd()):
    '''
    ...
    if plot_bool:
        ...
        n, bins, patches = plt.hist(x=frac_ambig_basecalls_by_sample[~in_outgroup_bool], bins=my_bins, color='#0504aa',
...
def compute_mutation_quality(Calls, Quals):
    ...
    for k in range(Nmuts):
        ...
        if len(np.unique(np.append(Calls[k, :],
                                   idx_for_N))) <= 2:  # if there is only one type of non-N (4) call, skip this location
            MutQual[k] = np.nan;
            MutQualIsolates[k, :] = 0;
        else:
            ...
            c = Calls[k, :];
            c1 = c[np.newaxis, :];
            c2 = c[:, np.newaxis]  # (1,NStrain) and (NStrain,1) broadcast to (NStrain,NStrain)
            q = Quals[k, :];
            q1 = q[np.newaxis, :];
            q2 = q[:, np.newaxis]  # -"-
            g = (c1 != c2) & (c1 != idx_for_N) & (
                        c2 != idx_for_N)  # no data ==4; boolean matrix identifying find pairs of samples where calls disagree (and are not N) at this position
            ...
            min_q = np.minimum(np.broadcast_to(q1, (NStrain, NStrain))[g],
                               np.broadcast_to(q2, (NStrain, NStrain))[
                                   g])  # broadcast_to creates a view (no copy) before boolean indexing
            MutQual[k] = np.max(
                min_q)  # np.max(np.minimum(q1[g],q2[g])) gives lower qual for each disagreeing pair of calls, we then find the best of these; NOTE: np.max > max value in array; np.maximum max element when comparing two arryas
...
def find_recombination_positions(my_calls, my_cmt, calls_ancestral, mut_qual, my_rg, distance_for_nonsnp,
                                 corr_threshold_recombination, save_plots_bool=False, dir_save_fig=os.getcwd()):
    '''
    ...
    filter_SNVs_not_ancestral_allele = (calls_ingroup != np.tile(calls_ancestral, (
    num_samples_ingroup, 1)))  # mutations must differ from the ancestral allele
    ...
    # Print results
    print("Number of recombination positions found: " + str(sum(nonsnp_bool)) + " of " + str(
        len(p_goodpos)) + " preliminary SNVs.")
...
def annotate_mutations(my_rg, p_gp, ancnti_gp, calls_gp, my_cmt_gp, fixedmutation_gp, mutQual_gp, promotersize):
    ...
    for i, pos in enumerate(p_gp):
        ...
        if mut_cds_indices[i] == int(mut_cds_indices[i]):  # Intragenic
            ...
            p_anno = annotation_genes[contig_idx - 1].iloc[int(mut_cds_indices[
                                                                   i]) - 1]  # first -1 bcs contigs indexed starting at 1; second -1 bcs cds_indices indexed at 1 but dataframe rows indexed at zero
            ...
            if p_anno.loc['strand'] == 1:  # fwd
                mut_annotations['nt_pos'] = mut_annotations['contig_pos'] - mut_annotations[
                    'loc1'] + 1  # positions on genome indexed at 1 (pos, loc1)
            elif p_anno.loc['strand'] == -1:  # rev
                mut_annotations['nt_pos'] = mut_annotations['loc2'] - mut_annotations[
                    'contig_pos'] + 1  # positions on genome indexed at 1 (pos, loc2)
            else:  # no strandedness
                mut_annotations['nt_pos'] = "."  # can happen eg. Crispr
            if mut_annotations[
                'nt_pos'] == ".":  # observed with special 'type's like crispr annotated (rare!); leads to no AA inference
                aan = 9999999
            else:
                aan = int((mut_annotations[
                               'nt_pos'] - 1) / 3) + 1  # get codon that harbours mutation (codons indexed starting at 1)
            ...
            codons_ls = []  # init
            aa_ls = []  # init
            if len(mut_annotations['sequence']) >= (aan * 3) and mut_annotations[
                'translation'] != ".":  # only for proteins
                codon = mut_annotations['sequence'][
                        aan * 3 - 2 - 1:aan * 3]  # -1 bcs sequence is indexed at 0 but positional argument aan is not
            ...
            mut_annotations['NonSyn'] = len(set(mut_annotations[
                                                    'AA'])) > 1  # indicates if a nonsynonymous mutation is *possible*, not necessarily if it happened

            # Determine if any observed mutations *did* change the codon
            mut_annotations[
                'AA_gt'] = ''  # init # for filling in amino acids corresponding to observed mutations relative to the ancestor
            if len(mut_annotations['AA']) < 4:
                mut_annotations['type'] = 'U'  # unknown mutation type
            # Fill in ancestral allele
            if np.unique(ancnti_gp[:, i])[0] != NTs_to_int_dict[
                'N']:  # only if ancestral allele is known; [0] ok bcs ancnti_gp should by definition never have > 1 unique element per row
                mut_annotations['anc'] = int_to_NTs_dict[np.unique(ancnti_gp[:, i])[0]]
                mut_annotations['nts'] = mut_annotations[
                    'anc']  # add ancestral allele to observed NTs; will add other NTs later
                if len(mut_annotations['AA']) == 4:
                    mut_annotations['AA_gt'] = mut_annotations['AA_gt'] + mut_annotations['AA'][
                        NTs_list_without_N_to_idx_dict[int_to_NTs_dict[
                            np.unique(ancnti_gp[:, i])[0]]]]  # the AA list order corresponds to NTs list!
            ...
            for j, callidx in enumerate(calls_gp[:, i]):  # loop across samples
                if fixedmutation_gp[j, i] == True:  # fixed mutations only
                    if calls_gp[j, i] != NTs_to_int_dict['N']:
                        ...
                        if len(mut_annotations['AA']) == 4:
                            mut_annotations['AA_gt'] = mut_annotations['AA_gt'] + mut_annotations['AA'][
                                NTs_list_without_N_to_idx_dict[int_to_NTs_dict[maNT_gp[j, i]]]]
                    elif calls_gp[j, i] == -1:  # if diverse (calls not a mutation), add minor and major call
                        mut_annotations['AA_gt'] = mut_annotations['AA_gt'] + mut_annotations['AA'][
                            NTs_list_without_N_to_idx_dict[int_to_NTs_dict[maNT_gp[j, i]]]]
                        mut_annotations['AA_gt'] = mut_annotations['AA_gt'] + mut_annotations['AA'][
                            NTs_list_without_N_to_idx_dict[int_to_NTs_dict[minorNT_gp[j, i]]]]
            if len(mut_annotations['AA']) == 4:
                mut_annotations['type'] = 'S'  # initialize as S; eventually overwritten below if N
            # Remove duplicates
            mut_annotations['nts'] = ''.join(
                OrderedDict.fromkeys(mut_annotations['nts']).keys())  # get only unique nucleotides and keep order
            mut_annotations['AA_gt'] = ''.join(
                OrderedDict.fromkeys(mut_annotations['AA_gt']).keys())  # get only unique AAs and keep order
            ...
            if int(mut_cds_indices[i]) > 0:
                ...
                mut_annotations['distance1'] = mut_annotations['contig_pos'] - p_anno.loc[
                    'loc2']  # how far back the previous gene is
                if p_anno.loc['strand'] == -1:
                    mut_annotations['distance1'] = mut_annotations['distance1'] * -1

            # Get info for gene after SNP (if any)
            if int(mut_cds_indices[i] + 0.5) <= annotation_genes[contig_idx - 1].shape[0] and \
                    annotation_genes[contig_idx - 1].shape[1] != 0:
                p_anno = annotation_genes[contig_idx - 1].iloc[int(mut_cds_indices[
                                                                       i])]  # first -1 bcs contigs indexed starting at 1; second -1 bcs cds_indices indexed at 1 but dataframe rows indexed at zero
                ...
                mut_annotations['distance2'] = p_anno.loc['loc1'] - mut_annotations[
                    'contig_pos']  # how far ahead the next gene is
                if p_anno.loc['strand'] == 1:  # second conditional to evade empty chr (?????)
                    mut_annotations['distance2'] = mut_annotations['distance2'] * -1

            # Determine mutation type
            if ('distance1' in mut_annotations and mut_annotations['distance1'] > (-1 * promotersize) and
                mut_annotations['distance1'] < 0) or (
                    'distance2' in mut_annotations and mut_annotations['distance2'] > (-1 * promotersize) and
                    mut_annotations['distance2'] < 0):
...
def generate_tree(calls_for_tree, treeSampleNamesLong, sampleNamesDnapars, refgenome, dir_output, filetag,
                  buildTree=False, writeDnaparsAlignment=False):
    '''
    ...
    if writeDnaparsAlignment:
        ...
        write_calls_sampleName_to_fasta(calls_for_tree, sampleNamesDnapars,
                                        timestamp + "_" + filetag + "_dnapars")  # timestamp_dnapars.fa > for dnapars...deleted later
...
def annotate_sampleNames(samplenames, locations_long_names, patients_sbj, visits_sbj, locations_sbj):
    ...
    for i, name in enumerate(extendend_sampleNames):
        extendend_sampleNames[i] = "S" + patients_sbj[i] + "_V" + str(visits_sbj[i]) + "_" + locations_long_names[
            locations_sbj[i]] + "_" + name
...
def build_table_for_tree_labeling(p_chr_table, treeSampleNamesLong, calls_for_tree, patient=""):
    ...
    if patient != "":
        subprocess.run(["rm -fr tree_counting/subject" + patient + " ; mkdir tree_counting/subject" + patient],
                       shell=True)
        with open("tree_counting/subject" + patient + "/for_tree_labeling.csv", 'w') as csv_file:
            ...
            for i in range(p_chr_table.shape[0]):
                csv_file.write(",".join(
                    np.append(np.array([str(p_chr_table[i, 0]), str(p_chr_table[i, 1])]), calls_for_tree[i,])) + "\n")
    else:
...
def write_mutation_table_as_tsv(mut_positions, mut_quality, sampleNames, annotation_mutations, calls_for_tree,
                                names_for_tree, tsv_filename):
    '''
...
def process_arrays(arr1, arr2, arr3, arr4, sample_num, arr5):
    ...
    column_modes = [
        np.unique(col)[0] if len(np.unique(col)) == 1 else (1 if len(col) == 0 else np.argmax(np.bincount(col))) for col
        in col_data_nonzero]
    column_second_nonzero_modes = [
        sorted(set(col[col != 0]), key=lambda x: np.count_nonzero(col[col != 0] == x))[-2] if len(
            set(col[col != 0])) > 1 else 0 for col in col_data_nonzero]
    # print('arr2:',arr2)
...
def cal_freq_amb_samples(all_p, my_cmt):
    ...
    # print(e)
    # exit()
    freq_arr, check_arr = process_arrays(my_cmt.major_nt, my_cmt.major_nt_freq, my_cmt.counts_major_max,
                                         my_cmt.counts_minor_max, my_cmt.major_nt.shape[0], my_cmt.minor_nt)
    freq_d = {}
    check_d = {}
    c = 0
    for p in my_cmt.p:
...
def generate_cnn_filter_table(all_p, filt_res, dpt, dlab, dprob, dir_output, cmt_p, dgap, my_cmt, min_cov_filt):
    ...
    for p in all_p:
        ...
        if int(fl) == 0 and int(tem_warr) == 0 and int(filt_l) == 0:
            filt[p] = ''
            continue
        o.write(str(p) + '\t' + fl + '\t' + warr[0] + '\t' + filt_l + '\t' + warr[1] + '\t' + str(
            dpt['qual'][p]) + '\t' + str(dpt['cov'][p]) + '\t' + str(dpt['maf'][p]) + '\t' + str(
            dpt['indel'][p]) + '\t' + str(dpt['mfas'][p]) + '\t' + str(dpt['mmcp'][p]) + '\t' + str(
            dpt['cpn'][p]) + '\t' + str(dpt['fix'][p]) + '\t' + recomb + '\t' + str(freq) + '\t' + gf + '\n')
    for p in cmt_p:
        if p in filt:
