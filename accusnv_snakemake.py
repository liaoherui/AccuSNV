...
def reset_exp_file(infile,outdir,uid,sfile,ref_dir,all_p,idle_p,tf_slurm,tem_dir,min_cov_filt,min_cov_samp,exclude_samp,greport,aligner_threads,aligner,samclip):
	...
	while True:
		...
		elif re.search('aligner_threads',line):
			o.write('aligner_threads: \"'+str(aligner_threads)+'\"\n')
		else:
			o.write(line+'\n')
	o.close()
...
def snakefile_modify(infile,uid,cp_env):
	o=open('sk_tem_'+uid,'w+')
	f=open(infile,'r')
	lines=f.read().split('\n')
	for line in lines:
		if not re.search('source',line):
			...
			if not re.search('source accusnv_sub',line):
				...
				o.write(out+'\n')
	o.close()
	os.system('mv sk_tem_'+uid+' Snakefile')



def snakefile_modify_bowtie2(infile,uid):
	o = open('sk_tem_' + uid, 'w+')
	f = open(infile, 'r')
	lines = f.read().split('\n')
	rgc=0 # count which genome.fasta
	for line in lines:
		if re.search('fasta.bwt',line):
			line=re.sub('.fasta.bwt','_bowtie2.1.bt2',line)
			o.write(line+'\n')
		elif re.search('bwa mem',line):
			if re.search('bwa mem',line) and re.search('fq2',line): # Pair-end reads
				out="            \"bowtie2 --threads {aligner_threads} -X 2000 --no-mixed --no-unal --dovetail -x {params.refGenome} -1 {input.fq1} -2 {input.fq2} -S {output.samA} 2> {log} ; \""
			else:
				out="            \"bowtie2 --threads {aligner_threads} --no-unal -x {params.refGenome} -U {input.fq1} -S {output.samA} 2> {log} ; \""
			o.write(out+'\n')
		else:
			if re.search('bwa',line):

				if re.search('bwa index',line):
					out=re.sub('bwa index','bowtie2-build -q',line)
					out=re.sub('{input.fasta}','{input.fasta} {params}',out)
				else:
					out = re.sub('bwa', 'bowtie2', line)
				o.write(out + '\n')
			else:
				if re.search('genome.fasta', line):
					rgc += 1
					if rgc == 2:
						out = re.sub('.fasta', '_bowtie2', line)
						o.write(out + '\n')
					else:
						o.write(line + '\n')
				else:
...
def snakefile_modify_samclip(infile,uid):
	o = open('sk_tem_' + uid, 'w+')
	f = open(infile, 'r')
	lines = f.read().split('\n')
	for line in lines:
		if re.search('bwa mem',line):
			line=re.sub('>',' | samclip --ref {params.refGenome} --max 0   >',line)
		o.write(line+'\n')
	o.close()
	os.system('mv sk_tem_' + uid + ' Snakefile')
...
def main():
	...
	if not cp_env=='':
		...
		os.system('cp Snakefiles_diff_options/Snakefile_prebuilt_env '+script_dir+'/Snakefile')
		snakefile_modify('Snakefile',uid, cp_env)
	...
	#exit()
	if not tf_slurm==1:
		os.system('cp config_files/config_with_slurm.yaml  ' + script_dir + '/config.yaml')
	sfile=process_input_sfile(input_file,out_dir,uid,tem_dir)
	reset_exp_file(script_dir+'/experiment_info.yaml',out_dir,uid,sfile,ref_dir,all_p,idle_p,tf_slurm,tem_dir,min_cov_filt,min_cov_samp,exclude_samp,greport,aligner_threads,aligner,samclip)
