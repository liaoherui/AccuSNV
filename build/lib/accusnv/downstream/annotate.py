...
Reads ``_snv_state.npz`` (from stage 1) and the reference dir; writes
``snv_table_unfiltered.tsv`` and ``snv_table_final.tsv``, and deletes stage 1's
...
def build_mutation_table( mut_positions, mut_quality, annotation_mutations, calls_for_tree, names_for_tree ):
    ...
    def flatten(column, separator, default):
        ...
        return [''.join(str(item) + separator for item in value) if type(value) == list else default
...
def annotate_main(args):
    ...
    merge_two_tables(dir_output + '/snv_table_filtered_tmp.tsv', annotations,
                     dir_output + '/snv_table_unfiltered.tsv')
...
def main():
    args = parse_args()
    accusnv_log.setup('annotate', args.log, args.full_log)
    annotate_main(args)
    # Every column of it is now in snv_table_unfiltered.tsv. It is absent on the >100k fast
    # path, and re-running this stage needs the calling stage re-run to recreate it.
    tmp = args.output_dir + '/snv_table_filtered_tmp.tsv'
