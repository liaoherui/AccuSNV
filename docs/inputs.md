...
AccuSNV does not take FASTQ paths directly. For each sample it looks in the `Path` folder for
files whose name starts with `FileName`, and finds the FASTQ files that match it.
...
* The mapping indexes (`genome.fasta.bwt`, `.amb`, `.ann`, `.pac`, `.sa` and `.fai`) are built
  during the first run. If the reference directory is read-only, build them yourself beforehand
  with `bwa index genome.fasta` and `samtools faidx genome.fasta`.
...
Annotations produced by [Prokka](https://github.com/tseemann/prokka) or [Bakta](https://github.com/oschwengers/bakta) work, as do the GFF files from NCBI genomes. AccuSNV reads the CDS features, their coordinates and strand, the locus tag, product description, protein ID, GO terms and the translated sequence, and it uses the FASTA to extract each gene's nucleotide sequence.
