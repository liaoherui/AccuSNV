...
aligner_threads = config["aligner_threads"]
...
def get_bt2qc_input(wildcards):
    ...
    bt2_logs = expand(
        outdir +
        "/1-Mapping/bwa/bwa_{sampleID}_ref_{reference}.txt",
        zip,
        sampleID=sampleID_clade,
        reference=reference_clade)
    return bt2_logs
...
if flag == "mapping":
    input_all.append(
        expand(
            outdir +
            "/1-Mapping/bwa/{sampleID}_ref_{references}_aligned.sorted.bam",
            zip,
            sampleID=SAMPLE_ls,
    ...
    input_all.append(
        expand(
            ...
            references=REF_Genome_ls,
            outgroup=OUTGROUP_ls))
    # input_all.append(expand(outdir+"/1-Mapping/bwa_qc/alignment_stats_ref_{references}.csv",references=set(REF_Genome_ls)))
...
if flag == "mapping" or flag == "all":
    ...
    # Indexes reference genome for bwa
    ...
    rule bwa:
        input:
            ...
            # put here, so rule bwa only executed after rule
            # refGenome_index done
            bwaidx = ancient(rules.refGenome_index.output.bwaidx)
        params:
            refGenome = REF_GENOME_DIRECTORY + "/{reference}/genome.fasta",
        output:
            samA = outdir + \
                "/1-Mapping/bwa/{sampleID}_ref_{reference}_aligned.sam",
        log:
            # necessary for bwaqc
            log = outdir + \
                "/1-Mapping/bwa/bwa_{sampleID}_ref_{reference}.txt",
        shell:
            ...
            "bwa mem -t {aligner_threads} {params.refGenome}  {input.fq1} {input.fq2}  > {output.samA}  ; "
            "echo 'bwa-pe' ; "
            "else "
            "bwa mem -t {aligner_threads}  {params.refGenome} {input.fq1} > {output.samA} ; "
            "echo 'bwa-se' ;"
            " fi"

    # Runs a QC script to summarize results of bwa mapping
    # rule bwaqc:
    #    input:
    #        get_bt2qc_input,
    #    output:
    #        alignment_stats = outdir+"/1-Mapping/bwa_qc/alignment_stats_ref_{reference}.csv",
    #    params:
    #        outfile_noextension = outdir+"/1-Mapping/bwa_qc/alignment_stats_ref_{reference}",
    #    conda:
    #        "envs/bwaqc.yaml",
    #    shell:
    #        "python3 {SCRIPTS_DIRECTORY}/bwaqc.py -s {spls} -r {wildcards.reference} -d {CURRENT_DIRECTORY} -o {params.outfile_noextension}"

    # Compresses SAM file into BAM file (and removes duplicate reads)
    rule sam2bam:
        input:
            samA = rules.bwa.output.samA,
        params:
            bamDup = outdir + \
                "/1-Mapping/bwa/{sampleID}_ref_{reference}_aligned_dups.bam",
            bamDupMate = outdir + \
                "/1-Mapping/bwa/{sampleID}_ref_{reference}_aligned_dups.mates.bam",
            bamDupMateSort = outdir + \
                "/1-Mapping/bwa/{sampleID}_ref_{reference}_aligned_dups.sorted.mates.bam",
            DupStats = outdir + \
                "/1-Mapping/bwa/{sampleID}_ref_{reference}_markdup_stats.txt",
        output:
            bamA = outdir + \
                "/1-Mapping/bwa/{sampleID}_ref_{reference}_aligned.sorted.bam",
            bamAidx = outdir + \
                "/1-Mapping/bwa/{sampleID}_ref_{reference}_aligned.sorted.bam.bai",
        shell:
    ...
    rule sam2bam_cleanup:
        ...
        output:
            outdir + \
                "/1-Mapping/bwa/{sampleID}_ref_{reference}_cleanup_done.txt",
